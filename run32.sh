...
NIM="nim --verbosity:0 --hints:off "$@" --cpu:i386 --passC:-m32 --passL:-m32"
...
custom_elf() {
  ...
  ld -m elf_i386 --gc-sections -e _start -T script32.ld -o payload nimcache/$1.o
  objcopy -j combined -O binary payload payload.bin
  ENTRY=$(nm -f posix payload | grep '^_start ' | awk '{print $3}')
  nasm -f bin -o $1_custom -D entry=0x$ENTRY elf32.s
  chmod +x $1_custom
  output $1_custom
}
